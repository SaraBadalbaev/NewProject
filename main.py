...
@app.get("/tasks/title/{title}", response_model=List[Task])
async def read_tasks_by_title(title: str):
    tasks = [task.to_dict() for task in db.collection("tasks").where("title", "==", title).stream()]
    return tasks
